...
<div class="row">
    ...
    @if ($user->privilege == 'Administrator' || $user->privilege == 'Team Lead' ||  $user->privilege == 'Client Services Manager' )
    <a href="/dashboard/documents/delete/{{$document->id}}" class="close" aria-label="Close"><span aria-hidden="true"><i class="fa fa-times"></i></span></a>
                    ...
                    <tbody>
                      @if (count($documents) != 0)
                        @forelse ($documents as $document)
                            <tr>
                            <td>
                           <input type="checkbox"> - <a href="/uploads/documents/{{$document->document_true_name}}" target="_blank">{{$document-> document_name}}</a>
                            </td>
                                ...
                                <div class="col-md-offset-6">
                                    @if ($user->privilege == 'Administrator' || $user->privilege == 'Team Lead' ||  $user->privilege == 'Client Services Manager' )
                                    <a href="/dashboard/documents/{{$document_category}}/edit/{{$document->id}}" class="btn btn-default btn-sm"  role="button">Edit</a>
                                    <a href="/dashboard/documents/{{$document_category}}/delete/{{ $document->id }}" class="btn btn-default btn-sm" onclick="return confirm('Are you sure?')" role="button">Delete</a>
...
<div class="row">
    <div class="col-md-8 col-md-offset-5 col-sm-8 col-sm-offset-1 col-xs-offset-3">{!! $documents->appends(['_token' => csrf_token()])->appends(['search' => $search])->render() !!}</div>
</div>
...
<tr>
<td></td>
<td>There don't seem to be any documents here.</td>
<td></td>
</tr>
</tbody>
</table>
